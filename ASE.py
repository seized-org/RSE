...
if yesandno >= "yes":
    ...
    with open("InsertEncryptedMessagesToHereInOrderToDecryptThem.abrcrypt", 'rb') as enc: # Reads file
        encryptedd = enc.read() # Will read the file once executing variable
    
    with open("InsertKeysToHERE.abrcrypt", 'rb') as keysfile:
        ...
        with open("InsertEncryptedMessagesToHereInOrderToDecryptThem.abrcrypt", 'wb') as dec:
...
if yesno >= "yes": # If user types "yes" it will move further:
    ...
    with open("SSE-DataStoring.abrcrypt", 'wb') as keyinfile: # Opening the Data file as keyinfile
...
if yes_nos >= "yes": # If user types yes, it will move further...
    ...
    with open("InsertKeysToHERE.abrcrypt", 'rb') as keysfile: # INSERT YOUR FERNET KEY BEFORE EXECUING THE DECRYPTION PART
