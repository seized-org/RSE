...
if startyesno >= 'yes':
    ...
    with open("ASE-DataStoring.abrupt9", "wb") as key: # Opens the data storing file
        gen_key = Fernet.generate_key()
        key.write(gen_key) # Writes the fernet key

    with open("ASE-DataStoring.abrupt9", "rb") as f:
    ...
    with open("ASE-DataStoring.abrupt9", "wb") as f: # Opening an existing file to store the fernet key for later use
        f.write(first_line_read) # Writes the first line / fernet key on the file
        print(time.strftime("%d/%m/%Y"),"[+] Saving Fernet Key In ASE_DataStoring.txt...")
if startyesno >= 'no':




    # Prints a notification that the program stored your ferent key in ASE_DataStoring.txt
    # It does that so you will be able to remember the key and also to use that key for futher msg's
    # IN ORDER TO DECRYPT MESSAGES, PLEASE PUT THE MESSAGES IN InsertEncryptedMessagesToHereInOrderToDecryptThem.abrupt9 AND FERNET KEYS IN InsertKeysToHERE.abrupt9 BEFORE TRYING TO DECRYPT MESSAGES!
    # AFTER DECRYPTING THE MESSAGE YOU WANTED TO DECRYPT, YOU'LL BE ABLE TO SEE THE DECRYPTED MESSAGE INSIDE InsertEncryptedMessagesToHereInOrderToDecryptThem.abrupt9 FILE!
...
#if yesandno >= "yes":
    #with open("InsertKeysToHERE.abrupt9", "rb") as fileskey:
...
if yesandno >= "yes":
    ...
    with open("InsertEncryptedMessagesToHereInOrderToDecryptThem.abrupt9", 'rb') as enc: # Reads file
        encryptedd = enc.read() # Will read the file once executing variable
    
    with open("InsertKeysToHERE.abrupt9", 'rb') as keysfile:
        ...
        with open("InsertEncryptedMessagesToHereInOrderToDecryptThem.abrupt9", 'wb') as dec:
...
if yesno >= "yes": # If user types "yes" it will move further:
    ...
    with open("SSE-DataStoring.abrupt9", 'wb') as keyinfile: # Opening the Data file as keyinfile
...
if yes_nos >= "yes": # If user types yes, it will move further...
    ...
    with open("InsertKeysToHERE.abrupt9", 'rb') as keysfile: # INSERT YOUR FERNET KEY BEFORE EXECUING THE DECRYPTION PART
